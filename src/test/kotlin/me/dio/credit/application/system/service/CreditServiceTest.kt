...
class CreditService(
  ...
  private fun validDayFirstInstallment(dayFirstInstallment: LocalDate): Boolean {
    return if (dayFirstInstallment <= (LocalDate.now().plusMonths(3))) true
    else throw BusinessException("Invalid Date")
  }

  private fun validNumberOfInstallments(numberOfInstallments: Int): Boolean{
    return if(numberOfInstallments <= 48) true
    else throw BusinessException("Maximum number of Installments is 48")
